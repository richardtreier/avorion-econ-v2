...
const FACTORY_DATAFILE = './avorion/productionsindex.lua';
const GOODS_DATAFILE = './avorion/goodsindex.lua';

const FACTORY_OUTPUT_FILE = './src/lib/avorion/data/factories.json';
const GOODS_OUTPUT_FILE = './src/lib/avorion/data/goods.json';
...
try {
  ...
  fs.writeFileSync(FACTORY_OUTPUT_FILE, JSON.stringify(factories, null, 4), 'utf8');
  fs.writeFileSync(GOODS_OUTPUT_FILE, JSON.stringify(goods, null, 4), 'utf8');
  console.log(`Done.`);
} catch (err) {
  console.log(err);
}
...
function loadGoods() {
  ...
  const goods = lines.map(processGoodsLuaLine).filter((g) => g !== null);
  return goods;
}
...
function transformFactory(data, index, goods) {
  ...
  const { cost, upgradeCost, inputTotal, outputTotal, productionTime } = getFactoryCosts(
    factory,
    goods
  );
  factory.cost = cost;
...
function getFactoryCosts(factory, goods) {
  const inputTotal = factory.inputs.map((i) => findPriceForGood(i, goods)).reduce(sum, 0);
  const outputTotal = factory.outputs.map((i) => findPriceForGood(i, goods)).reduce(sum, 0);
  ...
  const outputTime = factory.outputs.map((i) => getProductionTimeForGood(i, goods)).reduce(sum, 0);
